...
public class Grid extends JComponent implements KeyListener, MouseListener
{
  ...
  public Grid(int numRows, int numCols, int windowSize)
  {
    init(numRows, numCols, windowSize);
  }
  
  public Grid(String imageFileName, int windowSize)
  {
    BufferedImage image = loadImage(imageFileName);
    init(image.getHeight(), image.getWidth(), windowSize);
    showImage(image);
  ...
  private void init(int numRows, int numCols, int windowSize)
  {
    lastKeyPressed = -1;
    ...
    int cellSize = Math.max(Math.min(windowSize / getNumRows(), windowSize / getNumCols()), 1);    
